...
class ShowResearchPage extends AbstractPage
{
		...
		if (count($CurrentQueue) == 0) {
			...
			foreach($CurrentQueue as $ListIDArray)
			{
				...
				if($ListIDArray[4] != $PLANET['id'])
					$CPLANET		= $GLOBALS['DATABASE']->getFirstRow("SELECT ".$resource[6].", ".$resource[31].", technician_used, scientis_used, siege_on FROM ".PLANETS." WHERE `id` = ".$ListIDArray[4].";");
				else
					$CPLANET		= $PLANET;
	...
	private function AddBuildingToQueue($Element, $AddMode = true)
	{
		...
		if($ActualCount == 0)
		{
			...
			$costRessources		= BuildFunctions::getElementPrice($USER, $PLANET, $Element, !$AddMode, $BuildLevel);
	...
	private function getQueueData()
	{
		...
		foreach($CurrentQueue as $BuildArray) {
			...
			$costRessources		= BuildFunctions::getElementPrice($USER, $PLANET, $BuildArray[0], false, $BuildArray[1]);
			$elementTime    	= BuildFunctions::getBuildingTime($USER, $PLANET, $BuildArray[0], $costRessources);
			...
			$scriptData[] = array(
				...
				'endtime2' 			=> str_replace(' ', '&nbsp;', _date($LNG['php_tdformat'], $BuildArray[3]), $USER['timezone']),
				'tottime' 			=> gmdate("H:i:s", $elementTime),
				'starttime'			=> str_replace(' ', '&nbsp;', _date($LNG['php_tdformat'], ($BuildArray[3]-$elementTime)), $USER['timezone']),
				'accomplished'		=> (round(((TIMESTAMP - ($BuildArray[3]-$elementTime)) * 100) / ($BuildArray[3] - ($BuildArray[3]-$elementTime)), 2).''),
			);
		}
	...
	{
		global $PLANET, $USER, $LNG, $resource, $reslist, $CONF, $pricelist, $requeriments;
		...
		if($mode == 'space') {
			$elementIDs	= array(109,110,111,115,117,118,124);
		}elseif($mode == 'militar') {
		...
		foreach($elementIDs as $ID => $Element)
		{
			...
			$costRessources		= BuildFunctions::getElementPrice($USER, $PLANET, $Element, false, $levelToBuild+1);
		...
		if($mode == 'space'){
		$Tab = $LNG['NOUVEAUTE_273'];
		}elseif($mode == 'militar'){
		$Tab = $LNG['NOUVEAUTE_274'];	
		}else{
		$Tab = $LNG['NOUVEAUTE_272'];
		}
