...
class ShowPlayerCardPage extends AbstractPage
{
	...
	function show()
	{
		...
		$query 		= $GLOBALS['DATABASE']->getFirstRow("SELECT 
						...
						p.name, 
						s.tech_rank, s.tech_points, s.popu_rank, s.popu_points, s.build_rank, s.build_points, s.defs_rank, s.defs_points, s.fleet_rank, s.fleet_points, s.ownship_points, s.ownship_rank, s.total_rank, s.total_points, 
		...
		$this->tplObj->assign_vars(array(	
			...
			'date_of_birth'		=> date('d.m.y', $query['date_of_birth']),
			...
			'ownship_rank'   => pretty_number($query['ownship_rank']),
			'ownship_points' => pretty_number($query['ownship_points']),
			...
			'ddates'			=> date('d.m.y', $query['register_time']) ,
			...
			'sendMsg'	    => sprintf($LNG['NOUVEAUTE_657'], $query['username']),
			'pircent1'	    => $query['total_points'] == 0 ? 0 : round((100 / $query['total_points'] * $query['build_points']),2),
			'pircent2'	    => $query['total_points'] == 0 ? 0 : round((100 / $query['total_points'] * $query['tech_points']),2),
			'pircent3'	    => $query['total_points'] == 0 ? 0 : round((100 / $query['total_points'] * $query['fleet_points']),2),
			'pircent4'	    => $query['total_points'] == 0 ? 0 : round((100 / $query['total_points'] * $query['ownship_points']),2),
			'pircent5'	    => $query['total_points'] == 0 ? 0 : round((100 / $query['total_points'] * $query['defs_points']),2),
			'pircent6'	    => $query['total_points'] == 0 ? 0 : round((100 / $query['total_points'] * $query['popu_points']),2),
		));
