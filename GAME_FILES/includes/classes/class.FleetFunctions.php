...
class FleetFunctions 
{
	...
	public static function GetFleetRoomOwn($Fleet)
	{
		global $pricelist;
		$FleetRoom 				= 0;
		foreach ($Fleet as $ShipID => $amount)
		{
			$getShipInfo = $GLOBALS['DATABASE']->getFirstRow("SELECT shipFret FROM ".VARS_USER." WHERE varId = ".$ShipID.";");
			$FleetRoom		   += $getShipInfo['shipFret'] * $amount;
		}
		return $FleetRoom;
	}
	...
	public static function GetFleetMaxSpeedOwn ($Fleets, $Player)
	{
		$FleetArray = (!is_array($Fleets)) ? array($Fleets => 1) : $Fleets;
		$speedalls 	= array();
		
		foreach ($FleetArray as $Ship => $Count) {
			$getShipInfo = $GLOBALS['DATABASE']->getFirstRow("SELECT constructionTime, shipSpeed FROM ".VARS_USER." WHERE owerId = ".$Player['id']." AND varId = ".$Ship.";");
			$speedalls[$Ship] = $getShipInfo['shipSpeed']*8;
		}
	...
	public static function GetFleetConsumptionOwn($FleetArray, $MissionDuration, $MissionDistance, $FleetMaxSpeed, $Player, $GameSpeed)
	{
		$consumption = 0;
		...
		{
			$getShipInfo 		= $GLOBALS['DATABASE']->getFirstRow("SELECT constructionTime, shipSpeed FROM ".VARS_USER." WHERE owerId = ".$Player['id']." AND varId = ".$Ship.";");
			$ShipSpeed          = $getShipInfo['shipSpeed'];
			$ShipConsumption    = 5;
	...
	{	
		...
		$Details = $GLOBALS['DATABASE']->getFirstRow("SELECT * FROM ".USERS." WHERE `id` = '".$GetInfoPlanet['id_owner']."';");
		...
		if ($MissionInfo['planettype'] == 2) {
			...
			if (!$UsedPlanet) {
				...
				if (!$YourPlanet) {
					//if(isModulAvalible(MODULE_NEGOCIATE))
						$avalibleMissions[]	= 12;
					if(isModulAvalible(MODULE_MISSION_ATTACK) && $Details['immunity_until'] < TIMESTAMP && $USER['immunity_until'] < TIMESTAMP && $GetInfoPlanet['id_owner'] != Asteroid_Id)
						$avalibleMissions[]	= 1;
	...
	public static function CheckBashPortalAttack($Target)
	{
		global $USER;
		if(!BASH_ON)
			return false;
			
		$Count	= $GLOBALS['DATABASE']->getFirstCell("SELECT COUNT(*) FROM ".PORTALBASH."
		WHERE attackerId = ".$USER['id']." 
		AND planetId = ".$Target."  
		AND timeAttack > ".(TIMESTAMP - BASH_TIME).";");
		return $Count >= 1;
	}
	
	public static function sendFleet($fleetArray, $fleetMission, $fleetStartOwner, $fleetStartPlanetID, $fleetStartPlanetGalaxy, $fleetStartPlanetSystem, $fleetStartPlanetPlanet, $fleetStartPlanetType, $fleetTargetOwner, $fleetTargetPlanetID, $fleetTargetPlanetGalaxy, $fleetTargetPlanetSystem, $fleetTargetPlanetPlanet, $fleetTargetPlanetType, $fleetRessource, $fleetPopulation, $specialized = 0, $fleetStartTime, $fleetStayTime, $fleetEndTime, $fleetGroup = 0, $missleTarget = 0)
	{
		global $resource, $UNI, $pricelist;
		
		$getPlanet	 	= $GLOBALS['DATABASE']->getFirstRow("SELECT specialships FROM ".PLANETS." WHERE id = ".$fleetStartPlanetID.";");
			...
			if(!isset($pricelist[$ShipID])){
				if(!empty($getPlanet['specialships']))
				{
					...
					foreach($buildArray as $Item)
					{
						...
						if($ShipID == $Element){
							$planetQuery2[]	= $Element.','.floattostring(($Count - $ShipCount));
							$insideId = 1;
						}else{
							$planetQuery2[]	= $Element.','.floattostring($Count);
						}
					}
					if($insideId == 0)
						$QryUpdFleet2[]	= $ShipID.','.floattostring($ShipCount);
				}else{
					$planetQuery2[]	= $ShipID.','.floattostring($ShipCount);
				}
				$SQL	= "UPDATE ".PLANETS." SET specialships = '".implode(";", $planetQuery2)."' WHERE id = ".$fleetStartPlanetID.";";
				$GLOBALS['DATABASE']->query($SQL);
			}elseif(isset($pricelist[$ShipID])){
				$planetQuery[]	= $resource[$ShipID]." = ".$resource[$ShipID]." - ".floattostring($ShipCount);
			}
			
		}
		
		foreach($specialized as $ShipID => $ShipCount) {
			$fleetData2[]	= $ShipID.','.floattostring($ShipCount);
			$planetQuery[]	= $resource[$ShipID]." = ".$resource[$ShipID]." - ".floattostring($ShipCount);
		}
		
		$fleetData2 = empty($fleetData2) ? "" : implode(';',$fleetData2);
				
		if (!empty($planetQuery)){
			$SQL	= "UPDATE ".PLANETS." SET ".implode(", ", $planetQuery)." WHERE id = ".$fleetStartPlanetID.";";
		...
		$SQL	= "LOCK TABLE ".LOG_FLEETS." WRITE, ".FLEETS_EVENT." WRITE, ".FLEETS." WRITE, ".PLANETS." WRITE;
				   ...
				   fleet_population_308 = ".$fleetPopulation[308].",
				   fleet_population_309 = ".$fleetPopulation[309].",
				   specialized_array    = '".$fleetData2."',
