...
class DoJobCronjob
{
	...
	function run()
	{
		...
		$GLOBALS['DATABASE']->query("UPDATE ".TRANSORTLOG." SET `legal` = 0 WHERE `delete` > 0 ;");
		
		
		$query = $GLOBALS['DATABASE']->query("SELECT `senderID`, `receiverID` FROM ".TRANSORTLOG." WHERE `time` < ".(TIMESTAMP - 72 * 3600)." AND `delete` = 0 AND `legal` = 1;");
		
		while($x = $GLOBALS['DATABASE']->fetch_array($query)){
		$xD = $GLOBALS['DATABASE']->query("SELECT * FROM ".BANNED." WHERE longer > ".(TIMESTAMP)." AND who = '".$this->getUsernames($x['senderID'])."';");
		$xDs = $GLOBALS['DATABASE']->query("SELECT * FROM ".BANNED." WHERE longer > ".(TIMESTAMP)." AND who = '".$this->getUsernames($x['receiverID'])."';");
