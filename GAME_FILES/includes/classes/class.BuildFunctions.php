...
class BuildFunctions
{
	...
	public static function getMaxConstructibleOwn($USER, $PLANET, $Element, $cost901 = NULL, $cost902 = NULL, $cost903 = NULL, $cost904 = NULL)
	{
		global $resource, $reslist;
		
		if($cost901 > 0)
			$elementPrice[901]	= $cost901;
		if($cost902 > 0)
			$elementPrice[902]	= $cost902;
		if($cost903 > 0)
			$elementPrice[903]	= $cost903;
		if($cost904 > 0)
			$elementPrice[904]	= $cost904;
	...
	public static function getBuildingTime($USER, $PLANET, $Element, $elementPrice = NULL, $forDestroy = false, $forLevel = NULL)
	{
		...
		if	   (in_array($Element, $reslist['build'])) {
			$time	= $elementCost / Config::get('game_speed');
			$time	*= 1- ($MinTech/100);
			$time	*=  1-$Fastmode;
			if($Siege == 1){
			...
			$time	= $elementCost / (Config::get('game_speed') * (1 + $PLANET[$resource[14]] / 2) * (1 + $PLANET[$resource[21]] * 0.08));	
			$time	*=  1-$Fastmode;
			if($Siege == 1){
				for ($i = 0; $i < $PLANET[$resource[5]]; $i++) {
					$time	/=  1.50;
			...
			for ($i = 0; $i < $PLANET[$resource[46]]; $i++) {
				$time	/=  1.08;
			}
			...
			$time	= $elementCost / (1000 * (1 + $Level)) / (Config::get('game_speed') / 6000) * (1 + $USER['factor']['ResearchTime']);
			$time	*=  1-($MinScien/100);
			$time	*=  1-$Fastmode;
			if($Siege == 1){
				for ($i = 0; $i < $PLANET[$resource[5]]; $i++) {
			...
			for ($i = 0; $i < $PLANET[$resource[31]]; $i++) {
				$time	/=  1.025;
			}
	...
	public static function isElementBuyableOwn($USER, $PLANET, $cost901 = NULL, $cost902 = NULL, $cost903 = NULL, $cost904 = NULL)
	{
		$rest	= self::getRestPriceOwn($USER, $PLANET, $cost901 = NULL, $cost902 = NULL, $cost903 = NULL, $cost904 = NULL);
		return count(array_filter($rest)) === 0;
	}
	
	public static function getRestPriceOwn($USER, $PLANET, $cost901 = NULL, $cost902 = NULL, $cost903 = NULL, $cost904 = NULL)
	{
		global $resource;
		$elementPrice	= array();
		if($cost901 > 0)
