			...
			<div style="text-align : justify; padding : 5px 20px 10px 20px;">
				...
				{foreach $messageList as $payID => $messageRow}
				 <tr>
                    ...
                    <td class="mode_paiement cyan">{if $messageRow.pinAprouved == 0}<img src="https://cdngames.antaris-legacy.eu/media/ingame/image/mission/horloge.png" onmouseout="cache();" onmouseover="montre('Waiting on aprouval.');">{/if} {$messageRow.pinType}</td>
                    <td class="nb_credit">{$messageRow.pinCredits} <img src="https://cdngames.antaris-legacy.eu/media/ingame/image/credit.png" onmouseover="montre('Credits logo');" onmouseout="cache();"></td>
                    <td class="montant"><span class="couleur_theme">{$messageRow.pinPrice}</span> €</td>
                </tr>
				{/foreach}
				{else}
				<tr><td colspan="5" style="text-align : center;" class="vert">{$LNG.NOUVEAUTE_215}</td></tr>
				{/if}
				{if $messageList != ''}
				<tr class="total">
                <td colspan="3"></td>
                <td class="nb_credit">=&nbsp;{$messageRow.sum_pinCredits}&nbsp;<img src="https://cdngames.antaris-legacy.eu/media/ingame/image/credit.png" onmouseover="montre('Credits logo');" onmouseout="cache();"></td>
