...
<ul>		
	...
	<td style="text-align : left;">
							...
							</tr>		</table>	</div>	<div style="float : left; width : 47%; margin : 0px 10px 10px 10px;">		<h3>{$LNG.lm_statistics}</h3>		<table>			<tr>				<td style="text-align : left; font-weight : bold; width : 150px;">{$LNG.ls_alliance_109} :</td>				<td style="text-align : left;"><span class="couleur_theme">{$ally_members}</span> <span style="font-size : 0.8em;">{$LNG.ls_alliance_107}</span></td>			</tr>			<tr>				<td style="text-align : left; font-weight : bold;">{$LNG.ls_overview_33} :</td>				<td style="text-align : left;"><span class="couleur_theme">{$total_rank}</span> / {$allyCount}</td>			</tr>			<tr>				<td style="text-align : left; font-weight : bold;">{$LNG.ls_alliance_90} :</td>				<td style="text-align : left;"><span class="couleur_theme">{$total_points}</span> <span style="font-size : 0.8em;">{$LNG.NOUVEAUTE_18}</span></td>			</tr>			<tr>				<td style="text-align : left; font-weight : bold;">{$LNG.ls_alliance_108} :</td>				<td style="text-align : left;"><span class="couleur_theme">{$total_points_average}</span> <span style="font-size : 0.8em;">{$LNG.NOUVEAUTE_18}</span></td>			</tr>		</table>	</div>	<div class="espace"></div>	<div style="text-align : left; margin : 0px 10px 10px 10px;">		<h3><span onmouseover="montre('{$LNG.ls_alliance_110}');" onmouseout="cache();">DEFCON <sup>(?)</sup></span></h3>		<div class="centre">			<b>{$LNG.NOUVEAUTE_84} {$defcon} : </b> 
							{$defcontext}		</div>	</div></div><div class="categorie">	<h2>{$LNG.ls_alliance_106}</h2>	<div class="centre" style="margin-bottom : 10px;">{if $ally_description}{$ally_description}{else}{$LNG.NOUVEAUTE_82}{/if}</div></div><div class="categorie">	<h2>{$LNG.ls_alliance_111}</h2>	<div class="centre" style="margin-bottom : 10px;">{if $ally_text}{$ally_text}{else}{$LNG.NOUVEAUTE_83}{/if}</div></div></div></div>                    <!-- Fin du corps -->
