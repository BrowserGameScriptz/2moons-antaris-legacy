...
{if $tpanels == 1}
        ...
        <form id="form_pactiser" method="post" action="javascript:ActionMethode.envoyerFormulaire('form_pactiser', 'portail_teleportation', '?page=Tportal&mode=envoyerMission&onglet_page=pactiser');">
        <h2>{$LNG.NOUVEAUTE_549}</h2>
        
        <!-- Description de la mission -->
        <div name="description_mission">{$LNG.NOUVEAUTE_550}</div>
...
<script type="text/javascript">
        $(document).ready(function(){
            ...
            PortailTeleportation.initialiserOnglet('pactiser');
        });
</script>
{/block}