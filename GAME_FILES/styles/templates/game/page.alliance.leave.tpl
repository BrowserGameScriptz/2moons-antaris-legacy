...
<ul>		
	...
	<div class="categorie">	<h2>{$LNG.ls_alliance_68}</h2>	{$LNG.ls_alliance_69}.<br />
							{$LNG.ls_alliance_70}.	<br /><br />	
							
							<form id="form_ceder" action="game.php?page=alliance&amp;mode=admin&amp;action=transfer" method="post"> 	
							<label for="id_joueur">{$LNG.ls_alliance_71} : </label>		
							{html_options name=newleader options=$transferUserList}	
							<input type="submit" id="valid_form_ceder" name="valid_form_ceder" value="{$LNG.ls_alliance_72}" />	
							</form></div>
	{/if}
	<div class="categorie">	<h2>{$LNG.ls_alliance_73}</h2>	{$LNG.ls_alliance_74}.<br />	{$LNG.ls_alliance_75}.<br /><br />	
	
							<form id="form_suppr_alliance" action="game.php?page=alliance&mode=leaveAlly" method="post">	
							<input type="submit" id="valid_form_suppr_alliance" name="valid_form_suppr_alliance" value="{$LNG.ls_alliance_76}" />
							</form>
