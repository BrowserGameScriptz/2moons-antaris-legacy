	...
	<div id="div_profil_onglet"><div class="categorie">
					...
					{if empty($Message)}
						...
						<table>
								...
								<tr>
									{if !$sexe || $drates == 0}<td colspan="2"><input type="submit" name="valid_form" value="{$LNG.ls_settings_22}" /></td>{/if}
								</tr>
							</table>	</form>
							{else}
							<form id="form_donnees" method="post" action="game.php?page=settings">{$Message}<br>	<br><a onclick="location.href='game.php?page=settings';">{$LNG.NOUVEAUTE_166}</a>	</form>
							{/if}
