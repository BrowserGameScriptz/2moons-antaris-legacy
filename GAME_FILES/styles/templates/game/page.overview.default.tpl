...
<div id="salle_de_controle">
    ...
    <div name="couverture" data-nom_univers="horizon">
       ...
       <div name="aide_debutant">
			...
			{*<a href="https://cdngames.antaris-legacy.eu/media/documents/fr/howto.pdf" target="_blank" onmouseover="montre('{$LNG.NOUVEAUTE_266}');" onmouseout="cache();">{$LNG.NOUVEAUTE_267}</a>*}
 ...
 {elseif $userally != 0}
        ...
        <div name="bandeau_noir">
            ...
            <div name="information_joueur">
                <img name="avatar_joueur" src="styles/avatars/{$avatar}" />
    ...
    [{$ally_tag}]
                ...
                <div name="statistique">
                    ...
                    {$LNG.ls_overview_34} : <a href="javascript:SalleDeControle.toggleStatistique();" onmouseout="cache();"
                                        onmouseover="montre('{$LNG.ls_overview_35}.');">{$totalP}</a> {$LNG.NOUVEAUTE_18} 
                    <div data-js="stat_progression" >{if $cokies == 'false'}&mdash; 
                         {$postalP} {$LNG.NOUVEAUTE_18}{/if}</span>
                                            </div><br />
...
{if $allyID == 0}
		  <div {if $cokies == 'false'} style="display: block;"{else}style="display: none;"{/if} name="information_alliance" >
            ...
            <div {if $cokies == 'true'} style="display: block;"{else}style="display: none;"{/if} name="detail_statistique" ><!--
                --><div class="element">{$LNG.ls_overview_27} :    <span class="orange">{$totalBuild}</span> {$LNG.NOUVEAUTE_18}</div><!--
                --><div class="element">{$LNG.ls_overview_26} :    <span class="orange">{$totalOwnShips}</span> {$LNG.NOUVEAUTE_18}</div><!--
                --><div class="element">{$LNG.ls_overview_25} : <span class="orange">{$totalResearch}</span> {$LNG.NOUVEAUTE_18}</div><!--
                --><div class="element">{$LNG.ls_overview_24} :     <span class="orange">{$totalDefense}</span> {$LNG.NOUVEAUTE_18}</div><!--
				--><div class="element">{$LNG.NOUVEAUTE_655} :    <span class="orange">{$totalFleet}</span> {$LNG.NOUVEAUTE_18}</div><!--
                --><div class="element">{$LNG.ls_overview_23} :  <span class="orange">{$totalPopu}</span> {$LNG.NOUVEAUTE_18}</div><!--
            --></div>
        ...
        <div name="bandeau_news" onmouseover="SalleDeControle.pauseBandeauNews();" onmouseout="SalleDeControle.reprendreBandeauNews();">
            <ul data-js="defilement">
							   ...
							   <li name="numero_auto_1">{$LNG.ls_overview_22}</li>
								...
								<li name="numero_auto_3">{$journalLog}</li>
...
<div name="flotte" data-id_flotte="{$fleet.fleetID}" data-mission="{$fleet.mission}" data-statut="{if $fleet.fleetmes ==0}{$LNG.ls_ovbuild_15}{else}{$LNG.ls_ovbuild_16}{/if}">
    ...
    <div name="information_principale">
        <div name="mission">
            ...
            <span class="{if $fleet.missioncolor == 1 && $fleet.fleetOwner == $userID}vert{elseif $fleet.missioncolor == 1 && $fleet.fleetOwner != $userID}rouge{elseif $fleet.missioncolor == 3}chartreuse{elseif $fleet.missioncolor == 4}orange{elseif $fleet.missioncolor == 6}violet{elseif $fleet.missioncolor == 7}jaune{elseif $fleet.missioncolor == 8}marron{elseif $fleet.missioncolor == 12}cyan{elseif $fleet.missioncolor == 17}orange{/if}">{$fleet.mission}</span>
             — 
      ...
      <!-- Planète et joueur de départ -->
        ...
        --><div name="detail">
            ...
            <span onclick="javascript:SalleDeControle.afficherInformationFlotte({$fleet.fleetID});" style="cursor : pointer;" onmouseover="montre('{$LNG.NOUVEAUTE_695}');" onmouseout="cache();">
                <img src="https://cdngames.antaris-legacy.eu/media/ingame/image/info.png">
                           ...
                           {if $fleet.fleetmes ==0 && $fleet.fleetOwner == $userID}
						   <!-- Lien qui permet au joueur de demander le retour de sa flotte -->
                <a onclick="javascript:SalleDeControle.faireRetourFlotte('{$fleet.fleetID}')" onmouseover="montre('{$LNG.ls_detailflo_4}');" onmouseout="cache();"><img src="https://cdngames.antaris-legacy.eu/media/ingame/image/fleche_ronde.png"></a>{/if}
                    </div>
    </div> 
    ...
    <div style="display: none;" name="information_secondaire">
            ...
            {$fleet.fleetMission}<br>
            {$LNG.ls_detailflo_2} :                              {$LNG.NOUVEAUTE_696}<br>
...
{/foreach}
    ...
    {/if}
     <h2 class="titre_corps">
        ...
        {$LNG.ls_overview_15} {$planetname} [{$system}:{$planet}]
    </h2>
...
{$LNG.ls_overview_11}
 ...
 {if $buildInfo.tech}
                    ...
                    --></div>
                         {/if}  						 
 {if $buildInfo.fleet}
                            <h3>{$LNG.NOUVEAUTE_455} :</h3>
                                        
                    <!-- Affiche la liste des construction pour ce type d'entité -->
                                                            <div name="construction"><!--
                        --><img src="https://cdngames.antaris-legacy.eu/theme/gow/gebaeude/{$buildInfo.fleet['id']}.gif" /><!--
                        --><div name="nom">
                                                            
                             <span onmouseover="montre('<div style=\'min-width : 250px;\'>                <b>{$LNG.page_gestion_12} - {$LNG.tech[$buildInfo.fleet['id']]}</b> (<span class=\'orange\'>{$buildInfo.fleet['level']}</span>) <b>:</b>                    <ul style=\'text-align : left; margin : 0px; padding : 5px 5px 5px 20px;\'>                                {foreach $buildInfo.fleet['price'] as $RessID => $RessAmount}<li>{$LNG.tech.{$RessID}} : <span class=\'orange\'>{pretty_number($RessAmount*$buildInfo.fleet['level'])}</span> {$LNG.lm_achat_units}</li> {/foreach}                                            </ul>                <b>{$LNG.ls_ovbuild_3} :</b>    <ul style=\'text-align : left; margin : 0px; padding : 5px 5px 5px 20px;\'>                        <li>{$LNG.ls_ovbuild_1} : {$buildInfo.fleet['timelefts']}</li>        <li>{$LNG.ls_ovbuild_2} : {$buildInfo.fleet['endtime']}</li>            </ul></div>');" onmouseout="cache();" style="cursor : help;">{$LNG.tech[$buildInfo.fleet['id']]}</span>
                  ...
                  {if $buildInfo.defense}
                            <h3>{$LNG.NOUVEAUTE_461} :</h3>
                                        
                    <!-- Affiche la liste des construction pour ce type d'entité -->
                                                            <div name="construction"><!--
                        --><img src="https://cdngames.antaris-legacy.eu/theme/gow/gebaeude/{$buildInfo.defense['id']}.gif" /><!--
                        --><div name="nom">
                                                            
                             <span onmouseover="montre('<div style=\'min-width : 250px;\'>                <b>{$LNG.page_gestion_12} - {$LNG.tech[$buildInfo.defense['id']]}</b> (<span class=\'orange\'>{$buildInfo.defense['level']}</span>) <b>:</b>                    <ul style=\'text-align : left; margin : 0px; padding : 5px 5px 5px 20px;\'>                                {foreach $buildInfo.defense['price'] as $RessID => $RessAmount}<li>{$LNG.tech.{$RessID}} : <span class=\'orange\'>{pretty_number($RessAmount*$buildInfo.defense['level'])}</span> {$LNG.lm_achat_units}</li> {/foreach}                                            </ul>                <b>{$LNG.ls_ovbuild_3} :</b>    <ul style=\'text-align : left; margin : 0px; padding : 5px 5px 5px 20px;\'>                        <li>{$LNG.ls_ovbuild_1} : {$buildInfo.defense['timelefts']}</li>        <li>{$LNG.ls_ovbuild_2} : {$buildInfo.defense['endtime']}</li>            </ul></div>');" onmouseout="cache();" style="cursor : help;">{$LNG.tech[$buildInfo.defense['id']]}</span>
                    ...
                    --></div>
                         {/if}                                       
            {if $buildInfo.ownship}
                            <h3>{$LNG.NOUVEAUTE_456} :</h3>
                                        
                    <!-- Affiche la liste des construction pour ce type d'entité -->
                                                            <div name="construction"><!--
                        --><img src="{$buildInfo.ownship['image']}" /><!--
                        --><div name="nom">
                                                            
                             <span onmouseover="montre('<div style=\'min-width : 250px;\'>                <b>{$LNG.page_gestion_12} - {$buildInfo.ownship['nom']}</b> (<span class=\'orange\'>{$buildInfo.ownship['level']}</span>) <b>:</b>                    <ul style=\'text-align : left; margin : 0px; padding : 5px 5px 5px 20px;\'>                                {foreach $buildInfo.ownship['price'] as $RessID => $RessAmount}<li>{$LNG.tech.{$RessID}}  : <span class=\'orange\'>{pretty_number($RessAmount*$buildInfo.ownship['level'])}</span> {$LNG.lm_achat_units}</li> {/foreach}                                            </ul>                <b>{$LNG.ls_ovbuild_3} :</b>    <ul style=\'text-align : left; margin : 0px; padding : 5px 5px 5px 20px;\'>                        <li>{$LNG.ls_ovbuild_1} : {$buildInfo.ownship['timelefts']}</li>        <li>{$LNG.ls_ovbuild_2} : {$buildInfo.ownship['endtime']}</li>            </ul></div>');" onmouseout="cache();" style="cursor : help;">{$buildInfo.ownship['nom']}</span>
                             (<span class="orange">{$buildInfo.ownship['level']}</span>) :
                        </div><!--
                        --><div name="compteur" class="decompte_actualise"
                                data-time="{$buildInfo.ownship['timeleft']}">{$buildInfo.ownship['starttime']}</div><!--
                    --></div>
                         {/if}
                    </div><!--
        ...
        --><div name="planete">
            ...
            <span name="nom" onmouseover="montre('{$LNG.ls_overview_46} : {$planetname}');" onmouseout="cache();">
               {$planetname} [{$system}:{$planet}]
            </span><br />
       ...
       <!-- Affiche l'ensemble des planète de l'empire avec une scrollbar horizontal -->
		...
		{foreach from=$AllPlanetsBis item=i key=k}
                ...
                <div name="illustration">
                    <!-- Image de la planète -->
                    <img src="https://cdngames.antaris-legacy.eu/media/ingame/planet/{$i.planet}.jpg" onmouseover="montre('{$LNG.ls_overview_44}');" onmouseout="cache();" 
                ...
                <div name="information">
                    <a onclick="location.href='game.php?page=overview&cp={$i.id}';" class="nom_planete">{$i.name} [{$i.system}:{$i.planet}]</a><br />
