...
<ul>		
	...
	</ul></div><div id="div_alliance"><div class="categorie centre">	
	
	<h2>{$LNG.ls_alliance_91}</h2>	
	{if !empty($Message)}{$Message}<br><br>{/if}
	...
	<tr>			<td style="text-align : left;"><label for="defcon">DEFCON :</label></td>			<td style="text-align : left;">				<select id="defcon" name="defcon"><option value="1" {if $defcon == 1}selected{/if} >{$LNG.ls_alliance_96} n°1</option><option value="2" {if $defcon == 2}selected{/if}>{$LNG.ls_alliance_96} n°2</option><option value="3" {if $defcon == 3}selected{/if}>{$LNG.ls_alliance_96} n°3</option><option value="4" {if $defcon == 4}selected{/if}>{$LNG.ls_alliance_96} n°4</option><option value="5" {if $defcon == 5}selected{/if}>{$LNG.ls_alliance_96} n°5</option>				</select>			</td>		</tr>	
	...
	</table>	</form></div><div class="categorie centre">	
	<h2 onclick="ActionMethode.echangeAffichage('div_texte_plus'); ActionMethode.echangeAffichage('div_texte_moins');">{$LNG.ls_alliance_98}</h2>
	{if !empty($Message1)}{$Message1}<br><br>{/if}

	<div id="div_texte_plus" style="display : block;">		
...
<h2 onclick="ActionMethode.echangeAffichage('div_texte_interne_plus'); ActionMethode.echangeAffichage('div_texte_interne_moins');">{$LNG.ls_alliance_101}</h2>


	<div id="div_texte_interne_plus" style="">			<table style="width : 75%; margin : 0px auto 10px auto;">			<tbody><tr>
								<td style="text-align : left; padding-left : 4px;">
                        ...
                        <input class="bbcode" name="bbcode_souligne" value="U" onclick="insertionBBcode('form_tchat', 'tchat_message', '<u>', '</u>');" type="button">
							...
							</tr>		</table>		</form>	</div>	<div id="div_texte_interne_moins" style="display : block; margin-bottom : 10px;">	
							{if !empty($Message2)}{$Message2}<br><br>{/if}
							{$LNG.ls_alliance_100}.	</div></div></div></div>                    <!-- Fin du corps -->
