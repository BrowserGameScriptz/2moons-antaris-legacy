...
<ul>		
	...
	<form action="game.php?page=alliance&amp;mode=ranksissue" method="post">
<input type="hidden" value="1" name="send">
	...
	<td style="width : 170px; text-align : left; padding-left : 15px;">				<a href="game.php?page=alliance&amp;mode=ranksissue&amp;deleteRank={$rankID}" class="orange">{$LNG.ls_alliance_42}</a>			</td>	
	</tr>	
	{/foreach}
	</table>	
	...
	<div class="categorie">	<h2>{$LNG.ls_alliance_45}</h2>
	{if empty($Message)}
	{$LNG.ls_alliance_46}.	<br /><br />
	
	
	<form id="form_creer_rang" action="game.php?page=alliance&amp;mode=ranksissue" method="post">
	<label for="nom_rang">{$LNG.ls_alliance_47} : </label>		
	<input type="text" id="nom_rang" name="newrank[rankName]" style="margin-right : 10px; margin-left : 10px;" value="" size="20" maxlength="20" required/>	
	<input type="submit" id="valid_form_creer_rang" name="valid_form_creer_rang" value="{$LNG.al_create}" />
	</form>
	{else}
	<form id="form_notification" method="post" action="?page=alliance&amp;mode=ranksissue">{$Message}<br><a onclick="location.href='game.php?page=alliance&amp;mode=ranksissue';">{$LNG.NOUVEAUTE_21}</a>	</form>{/if}
