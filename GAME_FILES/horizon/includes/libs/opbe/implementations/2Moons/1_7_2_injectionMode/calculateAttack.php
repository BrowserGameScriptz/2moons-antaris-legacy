...
define('DEUTERIUM_ID', 903);
define('ELYRIUM_ID', 904);
 ...
 * @return array
 */
function calculateAttack(&$attackers, &$defenders, $FleetTF, $DefTF, $Small, $Large)
{
    ...
    foreach ($attackers as $fleetID => $attacker)
    {
        ...
        {
            if (empty($amount) || $element == 202 || $element == 203)
                continue;
    ...
    $debDefEly = 0;
    //total
    $debris = array('attacker' => array(METAL_ID => $debAttMet, CRYSTAL_ID => $debAttCry, DEUTERIUM_ID => $debAttDeu, ELYRIUM_ID => $debAttEly), 'defender' => array(METAL_ID => $debDefMet, CRYSTAL_ID => $debDefCry, DEUTERIUM_ID => $debDefDeu, ELYRIUM_ID => $debDefEly));
...
function getShipType($id, $count)
{
    ...
    $coque = $CombatCaps[$id]['coque'];
    $power = $CombatCaps[$id]['attack'];
    if ($id > ID_MIN_SHIPS && $id < ID_MAX_SHIPS)
    {
        return new Ship($id, $count, $rf, $shield, $coque, $power);
    }
    return new Defense($id, $count, $rf, $shield, $coque, $power);
}
