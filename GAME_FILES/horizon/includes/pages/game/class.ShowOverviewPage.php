...
{
	public static $requireModule = MODULE_OVERVIEW;
	...
	function show()
	{
		...
		if ($PLANET['b_building'] - TIMESTAMP > 0) {
			...
			$temp_total = (BuildFunctions::getBuildingTime($USER, $PLANET, $Queue[0][0], (BuildFunctions::getElementPrice($USER, $PLANET, $Queue[0][0], false, ($Queue[0][1])))));
			$temps_rest = $PLANET['b_building'] - TIMESTAMP;
			$pourc = 100-(100/$temp_total*$temps_rest);

			
			$buildInfo['buildings']	= array(
				'price'		=> BuildFunctions::getElementPrice($USER, $PLANET, $Queue[0][0], false, ($Queue[0][1])),
				'elementime'		=> pretty_time(BuildFunctions::getBuildingTime($USER, $PLANET, $Queue[0][0], (BuildFunctions::getElementPrice($USER, $PLANET, $Queue[0][0], false, ($Queue[0][1]))))),
				'id'		=> $Queue[0][0],
				'level'		=> $Queue[0][1],
				...
				'starttimeego'		=> date('d/m/Y H:i:s', ($PLANET['b_building'] - (BuildFunctions::getBuildingTime($USER, $PLANET, $Queue[0][0], (BuildFunctions::getElementPrice($USER, $PLANET, $Queue[0][0], false, ($Queue[0][1]))))))),
			);
			$overmessage = 1;	
		}
		else {
		...
		if ($USER['b_tech'] - TIMESTAMP > 0) {
			...
			$buildInfo['tech']	= array(
				...
				'starttimeego'		=> date('d/m/Y H:i:s', ($USER['b_tech'] - (BuildFunctions::getBuildingTime($USER, $PLANET, $Queue[0][0], (BuildFunctions::getElementPrice($USER, $PLANET, $Queue[0][0], false, ($Queue[0][1]))))))),
			);
			$overmessage = 1;
		}
		else {
		...
		$AllFeeds = array();
			...
			$allianceData = $GLOBALS['DATABASE']->GetFirstRow("SELECT a.id, a.ally_members, s.total_rank, s.total_points FROM ".ALLIANCE." as a INNER JOIN ".STATPOINTS." as s ON a.id = s.id_ally WHERE `universe` = 1 AND `id_ally` = ".$USER['ally_id'].";");
				
				
			$Total_alliance = $GLOBALS['DATABASE']->GetFirstCell("SELECT COUNT(*) FROM ".ALLIANCE.";");
			...
			if(!empty($allianceData)){
				$ally_members = $allianceData['ally_members'];
				$ally_points = pretty_number($allianceData['total_points']);
				$ally_rank = $allianceData['total_rank'];
			}
			
			
		$statinfo	= $GLOBALS['DATABASE']->query("SELECT s.total_points, s.popu_points, s.ownship_points, s.defs_points, s.build_points, s.fleet_points, s.tech_points, s.total_old_rank, s.total_rank, h.history_total_points, h.history_total_rank  FROM ".USERS." as u LEFT JOIN ".STATPOINTS." as s ON s.id_owner = u.id AND s.stat_type = '1' LEFT JOIN ".STATHISTORY." as h ON h.id_owner = u.id WHERE id = ".$USER['id'].";");
		while ($game = $GLOBALS['DATABASE']->fetch_array($statinfo)) {
			...
			$totalOwnShips 	= $game['ownship_points'];
			
			if($ranking == 0){
				$position = "<span style='color:orange'>0";
			}elseif($ranking < 0){
				$position = "<img src='/games/antaris/media/ingame/image/diminue.png' /><span style='color:rouge'>(".$ranking.")";
			}elseif ($ranking > 0){
				$position = "<img src='/games/antaris/media/ingame/image/augmente.png' /><span style='color:vert'>(+".$ranking.")";
			}
			
			if($totalo == 0){
				$positionP = "<span style='color:orange'>0";
			}elseif($totalo < 0){
				$positionP = "<img src='/games/antaris/media/ingame/image/diminue.png' /><span style='color:rouge'> (".pretty_number($totalo).")";
			}elseif ($totalo > 0){
				$positionP = "<img src='/games/antaris/media/ingame/image/augmente.png' /><span style='color:vert'> (+".pretty_number($totalo).")";
			}
		}
		...
		$GeTransportCount =$GLOBALS['DATABASE']->getFirstCell("SELECT COUNT(*) FROM ".TRANSORTLOG." WHERE senderID = ".$USER['id']." AND legal = '1' OR receiverID = ".$USER['id']." AND legal = '1';");
		...
		$journalLog = $GLOBALS['DATABASE']->getFirstRow("SELECT text FROM ".REGISTERLOG.";");
		...
		$this->tplObj->assign_vars(array(
			...
			'totalOwnShips' => pretty_number($totalOwnShips)	,
			'totalDefense' => pretty_number($totalDefense)	,
			'totalPopu' => pretty_number($totalPopu)	,
			'GetAll99' => $GetAll99	,
			'fleetCount' => $fleetCount	,
			'fleetCount1' => sprintf($LNG['NOUVEAUTE_750'], $fleetCount),
			'defcontext' => $defcontext	,
			'defcon' => $defcon	,
			'overmessage' => $overmessage	,
			'postal' => $post	,
			'postalP' => $postP	,
			'journalLog' => sprintf($LNG['NOUVEAUTE_795'], $journalLog['text']),
			'userRank' => $userRank	,
			'GeTransportCount' => $GeTransportCount	,
			'userTotal' => $userTotal,
			'Total_alliance'					=> $Total_alliance,
			'ally_tag'					=> $ally_tag,
			'ally_rank'					=> $ally_rank,
			'ally_points'					=> $ally_points,
			'ally_members'					=> $ally_members,
			'ally_memberss'					=> sprintf($LNG['NOUVEAUTE_452'], $ally_points, $ally_members),
			'ally_name'					=> $ally_name,
			'rankInfo'					=> $rankInfo,
