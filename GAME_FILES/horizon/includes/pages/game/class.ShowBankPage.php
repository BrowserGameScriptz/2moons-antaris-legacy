...
{
	public static $requireModule = MODULE_BANK;
	...
	function preview()
	{
		global $LNG, $CONF, $resource, $CONF, $USER, $PLANET;
		$this->initTemplate();
		$this->setWindow('ajax');
		...
		$Bankaccept = 0;
		$quantite_vendu = abs($quantite_vendu);
		...
		if($id_acheter == ""){
			$Bankaccept = 1;
			$Msg = $LNG['NOUVEAUTE_72'].'<span class="rouge" style="font-size : 12px;">'.$LNG['ls_bank_48'].'</span>';
		}elseif($id_vendu == ""){
			$Bankaccept = 1;
			$Msg = $LNG['NOUVEAUTE_72'].'<span class="rouge" style="font-size : 12px;">'.$LNG['ls_bank_49'].'</span>';
		}elseif($id_acheter == $id_vendu){
			$Bankaccept = 1;
			$Msg = $LNG['NOUVEAUTE_72'].'<span class="rouge" style="font-size : 12px;">'.$LNG['ls_bank_50'].'</span>';
		}elseif($result < 1){
			$Bankaccept = 1;
			$Msg = $LNG['NOUVEAUTE_72'].'<span class="rouge" style="font-size : 12px;">'.sprintf($LNG['ls_bank_53'],$quantite_vendu,$langsell,$langbuy,$langbuy).'</span>';
		}elseif($CONF['bank_'.$id_acheter] < $result){
			$Bankaccept = 1;	
			$Msg = $LNG['NOUVEAUTE_72'].'<span class="rouge" style="font-size : 12px;">'.sprintf($LNG['ls_bank_53'],$quantite_vendu,$langsell,$langbuy,$langbuy).'</span>';
		}elseif($taxSell < -15){
			$Bankaccept = 1;	
			$Msg = $LNG['NOUVEAUTE_72'].'<span class="rouge" style="font-size : 12px;">'.$LNG['ls_bank_51'].'</span>';	
		}elseif($taxBuy > 15){
			$Bankaccept = 1;	
			$Msg = $LNG['NOUVEAUTE_72'].'<span class="rouge" style="font-size : 12px;">'.$LNG['ls_bank_52'].'</span>';	
		}elseif($PLANET[$id_vendu] < $quantite_vendu){
			$Bankaccept = 1;	
			$Msg = $LNG['NOUVEAUTE_72'].'<span class="rouge" style="font-size : 12px;">'.sprintf($LNG['NOUVEAUTE_73'],pretty_number($PLANET[$id_vendu]),$langsell).'</span>';
		}
	...
	function sendSale()
	{
		global $LNG, $resource, $reslist, $CONF, $USER, $PLANET;
		
		$quantite_vendu = HTTP::_GP('quantite_vendu', 0);
		$CPSR = HTTP::_GP('cp', 0);
		
		if($CPSR != 0)
			$this->redirectTo('game.php?page=bank&mode=transaction');
		
		if(empty($quantite_vendu))
			$quantite_vendu = 0;
		$quantite_vendu = str_replace('+', '', $quantite_vendu);
		$quantite_vendu = str_replace(' ', '', $quantite_vendu);
		$type_vendu = HTTP::_GP('type_vendu', '');
		$id_vendu = HTTP::_GP('id_vendu', '');
		$type_acheter = HTTP::_GP('type_acheter', '');
		...
		if($PLANET[$id_vendu] < $quantite_vendu){
			header('Location: http://'.$_SERVER['HTTP_HOST'].'/game.php?page=bank&mode=transaction');
		}else{
		$taxSell = 0;
		$taxBuy = 0;
		if($id_vendu == 'metal'){
		...
		if($CONF['bank_'.$id_acheter] < $result){
			$this->redirectTo('game.php?page=Bank&mode=transaction');
		}
		...
		$LOG->pageLog  = "page=bank&mode=transaction";
		$LOG->info     = "The player sold ".pretty_number($quantite_vendu)." ".$langbuy." for ".pretty_number($result)." ".$langsell.".";
		$LOG->save();
