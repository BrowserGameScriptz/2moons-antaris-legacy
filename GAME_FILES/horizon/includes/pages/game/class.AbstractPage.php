...
abstract class AbstractPage 
{
	...
	protected function GetPlanetData() 
    {
		...
		$GetAll1 = $GLOBALS['DATABASE']->query("SELECT * FROM ".PLANETS." WHERE id_owner = ".$USER['id']." AND destruyed = '0' ORDER BY ".$Order.";");
	...
	protected function getPageData() 
    {
		...
		$url="https://makeyourgame.pro/onlinecount.php"; 
		$postdata = "useramount=".$balken."&prefix=".$database['tableprefix']."&method=save&onlineDomain=".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']); 
		...
		$GLOBALS['DATABASE']->query("INSERT INTO ".TRACKING." SET userId = ".$GLOBALS['DATABASE']->escape($USER['id']).", userName = '".$GLOBALS['DATABASE']->escape($USER['username'])."', pageVisited = '".$GLOBALS['DATABASE']->escape(empty($this->getQueryString()) ? 'game.php' : $this->getQueryString())."', time = ".TIMESTAMP.", trackMode = 1;");
        ...
        $this->tplObj->assign_vars(array(
			...
			'meta_descrip'		=> Config::get('meta_descrip'),
	...
	protected function printMessage($Message, $fullSide = true, $redirect = NULL) {
		...
		if($USER['user_deleted'] == 1 || $USER['banaday'] > TIMESTAMP || $CONF['game_disable'] == 0 || $USER['urlaubs_modus'] == 1 && $USER['urlaubs_until'] > TIMESTAMP){
