...
class FlyingFleetsTable
{
	...
	{
		global $LNG, $USER, $resource, $pricelist;
		...
		if ($this->IsPhalanx || $SpyTech >= 4 || $Owner)
		{
			...
			foreach($FleetRec as $Item => $Group)
			{
				...
				$Ship    = explode(',', $Group);
				$getUserShip = $GLOBALS['DATABASE']->getFirstRow("SELECT nom FROM ".VARS_USER." WHERE varId = ".$Ship[0].";");
				$LanguageName = !isset($pricelist[$Ship[0]]) ? $getUserShip['nom'] : $LNG['tech'][$Ship[0]];
				
				if($Owner) {
					$FleetPopup 	.= '<tr><td>'.$LanguageName.':</td><td>'.pretty_number($Ship[1]).'</td></tr>';
						$shipsData[]	= floattostring($Ship[1]).' '.$LanguageName;
				} else {
					if($SpyTech >= 8)
					{
						$FleetPopup 	.= '<tr><td>'.$LanguageName.':</td><td>'.pretty_number($Ship[1]).'</td></tr>';
					...
					{
						$FleetPopup		.= '<tr><td>'.$LanguageName.'</td></tr>';
						$shipsData[]	= $LanguageName;
					}
				}
			}
