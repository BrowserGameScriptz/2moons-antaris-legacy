...
$installSQL      = str_replace("%PREFIX%", $database['tableprefix'], $installSQL);
$GLOBALS['DATABASE']->multi_query($installSQL);
unlink('install.sql');
}
...
{	
	...
	$LNG->includeData(array('L18N', 'INGAME', 'TECH', 'CUSTOM', 'FLEET'));
	$THEME->setUserTheme($USER['dpath']);
	
	if(Config::get('game_disable') == 0 && $USER['authlevel'] == AUTH_USR) {
		ShowErrorPage::printError("<span class='rouge'>".$LNG['NOUVEAUTE_210'].'</span><br><br>'.Config::get('close_reason'), false);
	}
	if($USER['user_deleted'] == 1) {
		ShowErrorPage::printError(sprintf($LNG['NOUVEAUTE_208'],$USER['username']), false);
	}
	if($USER['urlaubs_modus'] == 1 && $USER['urlaubs_until'] > TIMESTAMP) {
		ShowErrorPage::printError(sprintf($LNG['NOUVEAUTE_205'], date('d/m/Y H:i:s', $USER['urlaubs_until'])), false);
	}
