{block name="title" prepend}{$LNG.NOUVEAUTE_839}{/block}
{block name="content"}
<div id="page_contenu"><h1>{$LNG.NOUVEAUTE_839}</h1>
	...
	<div id="" class="formulaire_mission categorie">
		<form action="?page=spaceSimulator" method="post">
	
	 <h2>{$LNG.NOUVEAUTE_839}</h2>
	 ...
	 <h3>{$LNG.NOUVEAUTE_832}</h3>
        ...
        <div name="conteneur_champ">
			...
			<div id="form_population">
				...
				{if $ownFleetCount == 0}<span class="rouge">{$LNG.NOUVEAUTE_841}</span>{else}
				{foreach $OwnShipsOnPlanet as $OwnRow} 
<div class="ressource">
						<img name="entite" src="{$OwnRow.image}" />
						<div class="conteneur_label">
							<label for="{$OwnRow.nom}">{$OwnRow.nom} :</label>
						</div>
						<input type="text" class="entier" id="ressource_{$OwnRow.id}" name="battleinput[0][0][{$OwnRow.id}]" data-appelation="{$OwnRow.nom}" maxlength="15" value="{if isset($battleinput.0.0.{$OwnRow.id})}{$battleinput.0.0.{$OwnRow.id}}{else}0{/if}" />
						<img name="maximum" src="https://cdngames.antaris-legacy.eu/media/ingame/image/mission/drapeau_bleu.png" onclick="insererChampFret('battleinput[0][0][{$OwnRow.id}]', {$OwnRow.count});" />
					</div>	
				{/foreach}
		...
		{/if}
		 ...
		 <div name="conteneur_champ">
			<h3>{$LNG.NOUVEAUTE_843}</h3>
			<div id="form_population">
				
				{if $ownFleetCountB == 0}<span class="rouge">{$LNG.NOUVEAUTE_844}</span>{else}
				
				{foreach $OwnShipsOnPlanetB as $OwnRow} 
				<div class="ressource">
						<img name="entite" src="{$OwnRow.image}" />
						<div class="conteneur_label">
							<label for="{$OwnRow.nom}">{$OwnRow.nom} :</label>
						</div>
						<input type="text" class="entier" id="ressource_{$OwnRow.id}" name="battleinput[0][1][{$OwnRow.id}]" data-appelation="{$OwnRow.nom}" maxlength="15" value="{if isset($battleinput.0.1.{$OwnRow.id})}{$battleinput.0.1.{$OwnRow.id}}{else}0{/if}" />
					</div>	
				{/foreach}
		...
		{/if}
		 ...
		 <div name="conteneur_champ">
			<h3>{$LNG.NOUVEAUTE_840}</h3>
			<div id="form_population">
				{foreach $defensiveList as $id}
				<div class="appareil">
					<img name="entite" src="https://cdngames.antaris-legacy.eu/theme/gow/gebaeude/{$id}.gif" />
					<div class="conteneur_label">
		...
		{if $ownFleetCount == 0}<span class="rouge">{$LNG.NOUVEAUTE_842}</span>{else}
					<input type="submit" id="valid_form" name="valid_form" value="{$LNG.NOUVEAUTE_829}" />{/if}
				</div>
				</div>
				</form>
				{else}
