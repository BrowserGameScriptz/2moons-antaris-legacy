...
<div id="liste_equipe">{foreach $userComp as $fleetID => $xb}<div id="emballage_equipe_{$fleetID}" class="emballage_equipe">	<div id="equipe_{$fleetID}" class="equipe">		<h2>{$LNG.NOUVEAUTE_475} : 
								...
								<img class="supprimer" src="https://cdngames.antaris-legacy.eu/design/image/jeu/icone/couleur/corbeille.png" onclick="javascript:VaisseauEquipe.supprimerEquipe('{$fleetID}');" />
							</h2>		<h3>{$LNG.NOUVEAUTE_485}</h3>		<div class="liste_vaisseau">	{foreach $xb.FLEETLIST as $shipID => $Element}<div class="item">
									...
									</div>{/foreach}</div><div class="option"><a class="modifier_couleur" title="{$LNG.NOUVEAUTE_487}" onclick="javascript:VaisseauEquipe.modifierCouleurEquipe('{$fleetID}');">{$LNG.NOUVEAUTE_487}</a></div></div></div>{/foreach}<div id="nouvelle_equipe" class="equipe">		<h2>{$LNG.NOUVEAUTE_483} <img class="icone_creer" src="https://cdngames.antaris-legacy.eu/design/image/jeu/icone/blanc/plus.png" /></h2>		<a onclick="javascript:VaisseauEquipe.afficherFormulaireCreation();" class="centre">{$LNG.NOUVEAUTE_483} !</a>	<form id="form_creer_equipe" action="javascript:VaisseauEquipe.creerEquipe();"><h3>{$LNG.NOUVEAUTE_485}</h3><div class="liste_vaisseau">{foreach $population as $PopRow}<div class="item"><img src="{$PopRow.image}" /><div class="element"><label for="vaisseau_{$PopRow.id}">{$PopRow.name}</label><input type="text" class="entier" id="vaisseau_{$PopRow.id}" name="vaisseau[{$PopRow.id}]" value="0" /></div></div>{/foreach}</div><h3>{$LNG.NOUVEAUTE_473}</h3><div class="liste_appareil">{foreach $appareil as $AppRow}<div class="item"><img src="/styles/theme/gow/gebaeude/{$AppRow.id}.gif" /><div class="element"><label for="appareil_{$AppRow.id}">{$AppRow.name}</label><input type="text" class="entier" id="appareil_{$AppRow.id}" name="appareil_{$AppRow.id}" value="0" /></div>
									</div>{/foreach}</div><div class="centre"><input type="submit" value="{$LNG.NOUVEAUTE_484}"/></div></form></div></div></div></div></div>                    <!-- Fin du corps -->
