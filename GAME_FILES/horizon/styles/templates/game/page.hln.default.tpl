...
<div id="fil_actualite">
	...
	{if !empty($Message)}<b>{$LNG.NOUVEAUTE_459} :</b> {$Message}<br>{/if}
	...
	<div id="option_show">
	{if $ShowText == 0}
	<form id="form_message" action="game.php?page=Hln" method="post"> 
			...
			{elseif $ShowText == 1}
			<span class="orange">{$LNG.ls_feed_21}</span> {*- <a onclick="">{$LNG.NOUVEAUTE_464}</a>*}
			{elseif $ShowText == 2}
    ...
    <h2 class="titre_corps">{$LNG.NOUVEAUTE_458}</h2>
...
{foreach $messageList as $messageID => $messageRow}
                ...
                <td class="message">
                    {$messageRow.message}
							...
							{foreachelse}
				<tr><td colspan="5" style="text-align : center;" class="rouge">{$LNG.ls_feed_20}</td></tr> 
				{/foreach}
