...
<ul>		
	...
	<div class="categorie">	<h2>{$LNG.ls_alliance_64}</h2>	{$LNG.ls_alliance_65}.<br>
				...
				{if empty($Message)}
						<form id="form_pacte" method="post" action="">			
						<label for="tag_pacte" style="margin-right : 20px;">{$LNG.ls_alliance_15} :</label>			
						<input id="tag_pacte" name="tag_pacte" size="8" maxlength="6" style="margin-right : 20px;" type="text">	
						<input id="valid_tag_pacte" name="valid_tag_pacte" value="{$LNG.ls_alliance_17}" type="submit">		
						</form>	
				{else}
						{$Message}<br><a onclick="location.href='game.php?page=alliance&mode=pactes';">{$LNG.NOUVEAUTE_107}</a>
				{/if}
