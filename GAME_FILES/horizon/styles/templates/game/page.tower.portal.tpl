...
<span class="cyan">{$teleport_portal_timer}</span>
{elseif $showMessagePortal == 2}
<form action="?page=Tower&mode=portal" method="post">
<input type="hidden" name="cmdd" value="desportal">
<input class="neutre" value="{$LNG.NOUVEAUTE_366}" type="submit">
</form>	
{elseif $showMessagePortal == 3}
<span class="cyan">{$teleport_portal_timer}</span>
{/if}
{elseif $isAllowedClose >= 3 && $teleport_portal == 0}
{if $showMessagePortal == 0}
<form action="?page=Tower&mode=portal" method="post">
<input type="hidden" name="cmdd" value="portal">
<input class="neutre" value="{$LNG.ls_tmanag_24}" type="submit">
...
{/if}
{else}
<span class="rouge">{$LNG.NOUVEAUTE_668}</span>
{/if}
...
<input class="neutre" value="{$LNG.NOUVEAUTE_360}" type="submit">
</form>	
{elseif $showMessageForce == 1}
<span class="cyan">{$LNG.NOUVEAUTE_363} : {$force_field_timer}</span>
{elseif $showMessageForce == 2}
<span class="cyan">{$force_field_timer}</span>
{/if}
				...
				<div id="statut_portail">{if $teleport_portal == 0}<span class="rouge">{$LNG.ls_tporal_3}</span>{else}<span class="vert">{$LNG.ls_tporal_4}</span> <b>/</b>{if $force_field_timer_show < $TIMESTAMP} <span class="rouge">{$LNG.ls_tporal_5}</span>{else} <span class="vert">{$force_field_timer_end}</span>{/if}
				{/if}
				</div>
				{/if}
