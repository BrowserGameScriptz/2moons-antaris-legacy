...
<div id="liste_equipe">{foreach $userComp as $fleetID => $xb}<div id="emballage_equipe_{$fleetID}" class="emballage_equipe">	<div id="equipe_{$fleetID}" class="equipe">		<h2>{$LNG.NOUVEAUTE_475} : 
								...
								<img class="supprimer" src="https://cdngames.antaris-legacy.eu/design/image/jeu/icone/couleur/corbeille.png" onclick="javascript:Population.supprimerEquipe('{$fleetID}');" />
							</h2>		<h3>{$LNG.ls_popu_1}</h3>		<div class="liste_population">	{foreach $xb.FLEETLIST as $shipID => $Element}<div class="item">
											...
											</div>	{if $Element@last}<div class="item ajouter_item" name="population">
									...
									{/if}{foreachelse}<div class="item ajouter_item" name="population">
										<img src="https://cdngames.antaris-legacy.eu/design/image/equipe_plus.png">
										<div class="element">
											<a title="{$LNG.NOUVEAUTE_481}" onclick="javascript:Population.afficherFormulaire('{$fleetID}', 'population');">{$LNG.NOUVEAUTE_482}</a>
											<form action="javascript:Population.ajouterElementEquipe('{$fleetID}', 'population');">
									...
									</div>{/foreach}</div><h3>{$LNG.NOUVEAUTE_473}</h3><div class="liste_appareil">{foreach $xb.APPAREILLIST as $shipID => $Element}<div class="item">
									...
									</div>{/foreach}</div><div class="option"><a class="modifier_couleur" title="{$LNG.NOUVEAUTE_474}" onclick="javascript:Population.modifierCouleurEquipe('{$fleetID}');">{$LNG.NOUVEAUTE_474}</a></div></div></div>{/foreach}<div id="nouvelle_equipe" class="equipe">		<h2>{$LNG.NOUVEAUTE_471} <img class="icone_creer" src="https://cdngames.antaris-legacy.eu/design/image/jeu/icone/blanc/plus.png" /></h2>		<a onclick="javascript:Population.afficherFormulaireCreation();" class="centre">{$LNG.NOUVEAUTE_471} !</a>	<form id="form_creer_equipe" action="javascript:Population.creerEquipe();"><h3>{$LNG.ls_popu_1}</h3><div class="liste_population">{foreach $population as $PopRow}<div class="item"><img src="https://cdngames.antaris-legacy.eu/media/ingame/population/{$PopRow.id}.jpg" /><div class="element"><label for="population_{$PopRow.id}">{$PopRow.name}</label><input type="text" class="entier" id="population_{$PopRow.id}" name="population_{$PopRow.id}" value="0" /></div></div>{/foreach}</div><h3>{$LNG.NOUVEAUTE_473}</h3><div class="liste_appareil">{foreach $appareil as $AppRow}<div class="item"><img src="https://cdngames.antaris-legacy.eu/theme/gow/gebaeude/{$AppRow.id}.gif" /><div class="element"><label for="appareil_{$AppRow.id}">{$AppRow.name}</label><input type="text" class="entier" id="appareil_{$AppRow.id}" name="appareil_{$AppRow.id}" value="0" /></div>
									</div>{/foreach}</div><div class="centre"><input type="submit" value="{$LNG.NOUVEAUTE_472}"/></div></form></div></div></div></div></div>                    <!-- Fin du corps -->
