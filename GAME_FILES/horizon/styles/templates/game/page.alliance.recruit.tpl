...
<div class="categorie">	<h2>{$LNG.ls_alliance_50}</h2>	{$ApplyCount}	<br>
						...
						<tr>				<td style="text-align : center; width : 8%; vertical-align : middle;">
											...
											<b>{$LNG.ls_alliance_31} : </b><span class="couleur_theme">{$messageRow.total_rank}</span> / {$userAmounts}
											...
											<a href="game.php?page=alliance&amp;mode=recruitAlly&amp;action=sendAnswerToApply&amp;id={$messageID}&answer=yes" class="vert">{$LNG.ls_alliance_55}</a> <br>
											<a href="game.php?page=alliance&amp;mode=recruitAlly&amp;action=sendAnswerToApply&amp;id={$messageID}&answer=no"class="rouge">{$LNG.ls_alliance_56}</a>
